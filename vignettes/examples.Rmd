...
The ANOVA power calculation here follows an [example](https://cran.r-project.org/web/packages/Superpower/vignettes/intro_to_superpower.html#an-example) of the [Superpower](https://github.com/arcaldwell49/Superpower) R package, for comparability.
...
Now the testing function will require a data frame to be contructed from the passed variables, to be appropriate for the `aov()` function. (Again, some wrapper function such as `ez::ezANOVA` could be used for readability, but this would make the simulation somewhat slower. However, if speed is of not much importance, those wrapper functions work just as well.)
...
This lengthy function pair is really good to verify via `do.call(testAOV, sampleAOV(100))`.
...
Importantly, of course, with `POSSA` there is the option for sequential testing. Here, it is particularly important to pay attention which specific one(s) of the included tests (p values) should have stopping alphas (or to have futility bounds).
...
Again, any different sample sizes could also be [specified]((https://gasparl.github.io/possa/vignettes/intro.html#specifying-initial-local-alphas)) for each of the two groups via n_obs (e.g. `n_obs = list(grp_1 = c(10, 15, 30), grp_2 = c(20, 45, 60))`).
...
Instead of the huge sample size of `90*4 = 360` for any decent SESOI (here: an AUC improvement of about 10%), the average required sample is reduced to 259.3 for H0 and292.2 for H1, with only a practically negligible `.007` reduction in power (`.787` instead of `.795`).
