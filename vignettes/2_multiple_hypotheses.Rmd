...
vignette: >
  %\VignetteIndexEntry{2_multiple_hypotheses}
...
This tutorial presupposes having read the [general introduction to `POSSA`](https://gasparl.github.io/possa/vignettes/1_intro.html). Here, that introduction is extended for cases of multiple hypotheses, i.e., where each analysis (at a given "look") involves multiple significance tests, and hence multiple p values.
...
Hence, recording descriptives and using varying factors will be explained here first, after which testing for multiple hypotheses is presented. This completes (together with the [intro](https://gasparl.github.io/possa/vignettes/1_intro.html)) everything important that needs to be known for using `POSSA`.
...
This is all needed to know to use `POSSA`. For further details about each function and each parameter, see the [manual](https://github.com/gasparl/possa/blob/master/POSSA.pdf). You also fine practical examples [here](https://gasparl.github.io/possa/vignettes/3_examples.html).
