...
This is equivalent to specifying each futility bound. Since futility bound is meaningless for the final look, and the present example (`df_ps_seq`) contains two interim looks, two values are required. Hence the `pow(df_ps_seq, fut_locals = c(0.5, 0.5))` gives results equivalent to `pow(df_ps_seq, fut_locals = 0.5)` (as above).
...
```r
pow(df_ps_seq, fut_locals = c(0.6, 0.3))
